...
// Uses the same CMS API episode-loading pattern as Audio Replace, then clicks
// the visible CMS UI for Use Current Audio -> Save Audio -> optional page Save.
(function(){
  ...
  const VERSION='2026-05-21.5-modal-save';
  ...
  async function fetchEps(showId){
    async function paged(view, chapPag){
      ...
      while(page<=2000){
        const params=[`show_id=${encodeURIComponent(showId)}`,'is_novel=0',`page_no=${page}`];
        if(view) params.push(`view=${view}`);
        if(chapPag) params.push('paginate_chapters=true');
        let r;
        try{
          r=await fetch(`${CMS}/book.show_episodes?${params.join('&')}`,{headers:hdrs(),credentials:'include'});
        }catch(e){
          break;
        }
    ...
    if(!combined.length){
      ...
      throw new Error('No episodes returned from CMS API - '+results.map((r,i)=>`${attempts[i][0]||'(none)'}${attempts[i][1]?'+chapPag':''}=${r.firstStatus}`).join(', '));
    }
  ...
  async function saveEpisodeNumber(ep,w){
    const seq=ep.seq||'?';
    log(`Opening Ep ${seq} (${ep.chapter_id.slice(0,8)}...)`);
  ...
  async function runAuto(){
    ...
    try{
      ...
      for(const ep of selected){
        ...
        try{
          await saveEpisodeNumber(ep,window);
          ok++;
        }catch(e){
          fail++;
  ...
  function render(){
    panel.innerHTML=`<div style="padding:12px 14px;background:#14532d;color:#fff;border-radius:12px 12px 0 0;display:flex;justify-content:space-between;align-items:center">
      <div>
        ...
        <div style="font-size:10px;color:#bbf7d0">CMS API list + visible click save · v${VERSION}</div>
      </div>
    ...
    <div style="padding:13px 14px">
      <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:9px;padding:10px 12px;margin-bottom:10px;color:#14532d;font-size:12px;line-height:1.45">
        Paste the CMS show ID or full CMS show URL, enter the episode range, then start. It loads episodes through the CMS API like Audio Replace, then clicks the visible CMS row to run Use Current Audio -> Save Audio.
      </div>
      ...
      <div style="font-size:11px;color:#64748b;line-height:1.45;margin-bottom:10px">
        Keep the target show open in this CMS tab. This version does not open a blank helper page; it uses CMS API data to decide which episode numbers to save.
      </div>
